...
public class ImageDAO extends AbstractDAO<Image> implements IImageDAO {
    ...
    public List<Image> findByHotelId(Integer id) {
        String sql = "SELECT * FROM Images WHERE Hotel_id = ?";
        List<Image> images = query(sql, new ImageMapper(), id);
        return images;
    }
    ...
    public List<Image> findByRoomId(Integer id) {
        String sql = "SELECT * FROM Images WHERE Room_id = ?";
        List<Image> images = query(sql, new ImageMapper(), id);
        return images;
    }
    ...
    public Integer save(Image Image) {
        String sql = "INSERT Images (ROOM_ID, HOTEL_ID, SLIDE_ID, LINK) VALUES(?, ?, ?, ?)";
        return insert(sql, Image.getRoom_id(), Image.getHotel_id(), Image.getLink());
    }

    @Override
    public void update(Image Image) {
        String sql = "UPDATE Images SET Room_id = ?, Hotel_id = ?, Slide_id = ?, Link = ? WHERE id = ?";
    ...
    public Image findBySlideId(Integer id) {
        String sql = "SELECT * FROM Images WHERE Slide_ID = ?";
        List<Image> images = query(sql, new ImageMapper(), id);
        return images.size() > 0 ? images.get(0) : null;
    }
}
